...
function zp_getPassCellValue($array, $key, $rates)
{
...
function buildPassFeesTable(array $data, array $rates)
{
    ...
    foreach ($data['prices']['fees'] as $fee) {
        ...
        foreach ($headers as $header => $key) {
            if ($key === 'zig_amount') {
                ...
                if ($strictUSD) {
                    // If strict_usd is true, set the ZWL amount to "N/A"
                    $formattedValue = 'N/A';
                } else {
                    // Get the USD amount
                    ...
                    // Calculate the amount in ZWL
                    $amountInZWL = $usdAmount * $zipitRate;
                    // Format the amount in ZWL with currency symbol and space separators
                    $formattedValue = number_format($amountInZWL, 0, '.', ' ') . ' ZiG';
                }
            } else {
                // For other keys, get the formatted value using zp_getPassCellValue function
                $formattedValue = zp_getPassCellValue($fee, $key, $rates);
            }
    ...
    $tableHTML = '<h4>Current Passport Application Fees: ' . htmlspecialchars(zp_today_full_date()) . '</h4>
